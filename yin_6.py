...
# ================== PARAMETER ==================
FRAME_INTERVAL_MS = 50          # resolusi waktu sampling (ms) → lebih rapat
...
F0_MIN_HZ = 50                  # batas bawah frekuensi masuk akal (ubah sesuai instrumen)
F0_MAX_HZ = 2000                # batas atas frekuensi masuk akal
# ===============================================
...
# 1) Pastikan nilai f0 di luar rentang wajar dihapus
valid = ~np.isnan(f0)
f0[(valid) & ((f0 < F0_MIN_HZ) | (f0 > F0_MAX_HZ))] = np.nan

# 2) Energy-based gating: gunakan RMS per frame
...
# Samakan panjang RMS dengan f0 (trim/pad bila perlu)
rms = librosa.util.fix_length(rms, size=len(f0))

# Ambil threshold dari persentil (misal 20% terendah = noise)
rms_threshold = np.percentile(rms, RMS_NOISE_PERCENTILE)

# Tandai frame dengan energi rendah sebagai tidak bersuara
low_energy_mask = rms < rms_threshold
f0[low_energy_mask] = np.nan

# Opsional: tampilkan info threshold
if SHOW_DEBUG:
    print(f"[NOISE] RMS threshold @ p{RMS_NOISE_PERCENTILE}: {rms_threshold:.6f}")
    num_low = np.sum(low_energy_mask)
    print(f"[NOISE] Frame low-energy: {num_low}/{len(f0)} ({num_low/len(f0)*100:.1f}%)")
...
# --- 8. Plot: line untuk note berlanjut, dot untuk transisi ---
...
for onset in onset_times:
    plt.axvline(onset * 1000, color='red', linestyle='--', alpha=0.5, linewidth=1)
