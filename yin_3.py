...
# === 4.5. Cari nada yang paling sering muncul ===
valid_notes = [n for n in notes if n != 'N/A']
if valid_notes:
    note_count = Counter(valid_notes)
    most_common_note, count = note_count.most_common(1)[0]
    percent = (count / len(valid_notes)) * 100

    # Ambil F0 rata-rata untuk note dominan
    f0_for_note = [f for f, n in zip(f0, notes) if n == most_common_note]
    f0_mean = round(np.mean(f0_for_note), 2)

    print(f"🎯 Frekuensi rata-rata untuk {most_common_note}: {f0_mean} Hz")
    print(f"🎶 Nada paling dominan: {most_common_note} ({percent:.1f}% dari {len(valid_notes)} frame valid)")
else:
...
dominant_text = f" | Nada Dominan: {most_common_note} ({percent:.1f}%), F₀ Rata-rata: {f0_mean} Hz"
